...
#' }
CrossICCshiny <- function(..., study.names, filter.cutoff = 0.5, fdr.cutoff = 0.1, output.dir = NULL, max.iter = 20, max.K = 6, rep.runs = 1000,
                     pItem=0.8, pFeature=1, clusterAlg="hc", distance="euclidean",
                     cc.seed=5000, cluster.cutoff = 0.05, ebayes.cutoff = 0.1, progress = NULL){
  ...
  progress$set(value = 10, detail = 'Pre-processing data')
  ...
  progress$set(value = 0.15, detail = 'Pre-processing data done ')
  step=round(0.85/max.iter,digits = 2)
  while(iteration <= max.iter){
    progress$set(value = 0.1515+step, detail = paste('Iteration',iteration))
